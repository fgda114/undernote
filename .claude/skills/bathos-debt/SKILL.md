---
name: bathos-debt
description: .agent-team/ 산출물과 _state/ 리스크 로그 전반의 "CONCERNS:" 앵커를 한 번에 수집해 부채 ledger로 보여준다. 게이트 판정 CONCERNS(조건부 통과)의 후속 해소 여부를 놓치지 않기 위함. "부채 확인", "/bathos-debt", "CONCERNS 목록", "남은 리스크 뭐 있어", "부채 ledger" 요청 시 사용.
---
...
> **재정의(카피 금지 준수):** ponytail-debt는 소스코드의 `ponytail:` 주석을 grep한다.
> BATHOS 산출물은 마크다운 문서이므로, 앵커를 `CONCERNS:` 리터럴(게이트 판정 §2.1의
> CONCERNS 표기와 동일 어휘)로 재정의한다. 읽기 전용 — 아무것도 변경하지 않는다.
...
```json
{
  ...
  "scanned_files": 37,
  "items": [
    { "id": "D-01", "wave": "W3", "severity": "concerns",
...
| 빈(0건) | `✓ 미해결 CONCERNS 없음 — 부채 0건 (scanned N files)` — **성공 톤**(문제 없음을 긍정적으로) |
