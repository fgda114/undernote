...
- **결정 (리드 Paul, 2026-09-02): ⓐ 채택 — 1차 공개 시 초과 수용.** 근거: 초과분 전부 폰트(재방문 0KB·1.5MB 캐시 예산 내), 유입 축은 커뮤니티 유통·SNS라 첫 방문 이탈보다 읽기 경험 우선, 500KB 자체가 실측 없는 값.
- **단 예산은 유지된다 — 트래픽 실측 후 재설정한다**: GoatCounter 데이터로 실제 이탈을 확인한 뒤, 필요하면 unicode-range 분할(반나절)을 집행한다. **재검토 시점: 첫 3개월 또는 방문 지표 확보 시.**
...
- [ ] **픽스처 콘텐츠 제거 + 실콘텐츠 최소 세트 투입** (원의 첫 평론) — 제거 대상: content/{albums,reviews,artists,stories}/fixture-* · public/covers/fixture-*. **잔존 시 배포가 자동 차단됨** — CI deploy 잡의 릴리스 한정 게이트(리드 결정 ②: dev/verify는 초록 유지, 배포 경계에서만 검사·재빌드 없음)
- [x] 성능 예산 협의 → **결정 완료 (§1 — 1차 수용 + 실측 후 재설정)**
