...
## W5.5 — 정체성 + 공개 게이트 (2026-09-02)

- /about/ 완성(3문 소제목 + 권리 연락 경로) · /404/ 신규 — 화면 8종+404 전 라우트 완성.
- 성능 실측·계측 선정(GoatCounter)·§5 자가 점검·운영 수칙·게이트 잔여 체크리스트 → **`w5-5-launch-gate.md`** 참조 (이 스토리의 본체 문서).
- 성능 예산 협의 부의: 초기 전송 754KB~1,067KB > 500KB (폰트 3면이 전부 — 재방문 캐시 0KB). 계측 설치는 User 계정 + §7 필드 대기.
- 상태: **review** — 개발분 완료, Matthias QA 전수(US-1~15)·User 작업 4건·예산 협의가 공개 게이트 잔여.

---

## W5.4 — 이해의 사다리 (2026-09-02)

### 주요 결정·발견 (다음 스토리 필독)

1. **계보 선택 상태: 원 무응답 → 안 A 확정 구현.** AlbumBox만 렌더. LineageBlock은 예비 계약 상태로 W5.1 시안 컴포넌트 없음(§12b는 활성화하지 않음 — role 필드는 스키마에 있고 소비만 미결선). 원이 안 B를 고르면: derive에 role 그룹핑 추가 + LineageBlock 컴포넌트 신작이 필요 (반나절 규모).
2. 폴백 사슬은 `derive/links.ts` — 첫 비지 않는 단계만, 태그/버킷 최대 2, direct 전부 발행 역순. 페이지는 mode만 보고 리드 카피 선택(tag는 direct 카피 재사용 — W3 N-8 결정 반영).
3. **본문 자동 이름 매칭 링크는 만들지 않음** (스토리 명시 해석 — US-4 AC1은 AlbumBox 링크로 충족, 원이 쓴 마크다운 링크는 그대로 렌더).
4. "{아티스트}의 다른 글" = W5.3 by_artist 집계 재사용, 복수 아티스트 합집합·현재 평론 제외·중복 제거.
5. 소급 링크는 전량 재계산의 공짜 속성 — 테스트가 양방향(이야기 행 승격 + 평론 백링크 생성)을 고정.
6. 이야기 H1 29px도 계약 고정값 — `--fs-29` 토큰 추가 (14·16과 같은 계열).
