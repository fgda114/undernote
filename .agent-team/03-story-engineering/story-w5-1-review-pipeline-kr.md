...
```
수정: src/content.config.ts        # lib/schema 단일 정의를 감싸는 wrapper로 (N-2 리드 확정)
신규:
  src/lib/schema/                  # Zod 8스키마 단일 정의 (순수 모듈·한국어 메시지) — content.config.ts와 checker가 공동 소비
  src/lib/score.ts                 # 유일한 점수 파서
  src/lib/checker/                 # E-100~109 · E-201~205 (E-110~115는 W5.2) — lib/schema 소비, 재정의 금지
  ...
  public/favicon.svg · public/apple-touch-icon.png   # seal 도트 모티프 (N-9)
