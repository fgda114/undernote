...
**편집 CLI(`scripts/album-add.ts`)가 편집 시점에 1회 조회한다:**
...
3. **실패 시 수기 폴백** — CLI가 빈 필드를 명시한 앨범 YAML을 생성하고 User가 채운다. 발매 연도 공백은 빌드 실패(E-102). MB 지연 반영(B6)은 수기 선입력 + 추후 `mbid` 연결 허용 (SS-2).
